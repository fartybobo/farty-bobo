...
description: Post a footer or inline comment on a Confluence page using the Atlassian MCP server. Accepts a Confluence page URL or a direct comment URL. The comment is attributed to Gossip Girl.
---
...
**Comment opening:** Prepend `Gossip Girl {VERB}:` as the very first line of the final comment, followed by a blank line, then the body. If the caller-provided body already starts with `Gossip Girl`, do not prepend a second header. Choose the verb based on context:
...
- **Verb chosen:** e.g. `Gossip Girl commented:`
...
Re-apply the `Gossip Girl {VERB}:` header and identity footer to any revised body, then re-show the full preview. Repeat until the human confirms or cancels.
